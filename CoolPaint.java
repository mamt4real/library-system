...
public class CoolPaint implements ClockPaint{
	...
	private final static int AM_0_00 = 0;
	private final static int AM_3_00 = 3 * SECONDS_IN_HOUR;
	private final static int AM_4_30 = 4 * SECONDS_IN_HOUR + SECONDS_IN_HALF_HOUR;
	private final static int AM_7_30 = 7 * SECONDS_IN_HOUR + SECONDS_IN_HALF_HOUR;
	private final static int AM_12_00 = 12 * SECONDS_IN_HOUR;
	private final static int PM_4_30 = 16 * SECONDS_IN_HOUR + SECONDS_IN_HALF_HOUR;
	private final static int PM_7_30 = 19 * SECONDS_IN_HOUR + SECONDS_IN_HALF_HOUR;
	private final static int PM_9_00 = 21 * SECONDS_IN_HOUR;
	private final static int PM_12_00 = 24 * SECONDS_IN_HOUR;
	...
	private static final Color[] COLOR_CYCLE = {BLACK,DARK_GRAY,DARK_BLUE,PURPLE,CYAN,YELLOW,PALE_YELLOW,RED,GREEN,LIGHT_BLUE,SKY_CYAN	}; 
	
	private static final int RADIAL_PERIOD_LENGTH = PM_12_00 / COLOR_CYCLE.length;
	
	private static final String[] ROMAN = {"","V","IV","III","II","I","XII","XI","X","IX","VIII","VII","VI"};
	...
	private static class Painter{
		...
		private Color mixColors(Color startColor, Color endColor,int startTime, int endTime, int currentTime){
			...
			return new Color(mixColorComponent(startColor.getRed(),endColor.getRed(),normalized),
		...
		private RadialGradientPaint colorOnCycle(Point2D center, float radius){
			...
			float[] positions = index3== 1.0? new float[] {0.0f,index2,1.0f} : new float[] {0.0f,index2,index3,1.0f};
			Color[] colors = index3 == 1.0? new Color[] {start, baseColor2, end	} : new Color[] {start, baseColor3,baseColor4, end	};
		...
		private void paintNumbers(){
			...
			double amplification = (int) Math.max(radius*0.8, originalFont.getSize())/ (double) originalFont.getSize();
